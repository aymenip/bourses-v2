...
export const createField = async (createFieldDTO: CreateFieldDTO): Promise<FieldDTO> => {

    try {
        const result = await (await db).insert(fields).values(createFieldDTO).execute();
...
export const updateField = async (updateFieldDTO: UpdateFieldDTO): Promise<FieldDTO> => {
    try {
        const dbInstance = await db;
        const result = await dbInstance
            .update(fields)
            .set({})
            .where(and(eq(fields.id, updateFieldDTO.id), eq(fields.formId, updateFieldDTO.formId)))
            .execute();
        const field = await getFieldById(result[0].insertId);
        return field;
    } catch (error) {
