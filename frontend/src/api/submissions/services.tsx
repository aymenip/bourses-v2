...
import { TCreateSubmission, TSubmission } from "@/types/submissions";
...
export async function getSubmissionsForUser(positionId: number): Promise<TSubmission[]> {
    const token = authenticationContext().token;
    ...
    const response = await axiosInstance.get<TSubmission[]>(`submission/user/${positionId}`, {
        headers: {
            Authorization: token,
        },
    });
